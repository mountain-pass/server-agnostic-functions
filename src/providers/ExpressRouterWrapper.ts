...
export const wrap = (agnosticRouter: AgnosticRouter) => {
        ...
        expressRouter.use(async (req: Request, res: Response) => {
            try {
                ...
                if (req.headers) Object.entries(req.headers).forEach(([key, value]) => {
                    request.headers[key.toLowerCase()] = Array.isArray(value) ? value : [value]
                })
                if (req.query) Object.entries(req.query).forEach(([key, value]) => {
